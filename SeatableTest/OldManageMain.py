...
existing_common_model_list = model_base.list_rows('普通配件')
existing_special_model_list = model_base.list_rows('重要/核心配件')
increased_common_record_list = record_base.list_rows('维修记录')
increased_pic_record_list = record_base.list_rows('维修记录(图片版)')
"""
由于表单中日期没有被设置为必填项，所以可能record_row里没有日期项。
对于没有日期项的行，先填写日期为当天，再重新提取新增的维修记录数据。
"""
# 建立更新故障日期的数据集容器。
increased_common_record_update_data = []
# 获取当前日期字符串。
now = datetime.now()
now_date_str = now.strftime('%Y-%m-%d/')

# 如果某行中没有日期key。
for examine_date_row in increased_common_record_list:
    if not ('故障日期' in examine_date_row):
        # 对该行数据集中添加当前日期。
        increased_common_record_update_data.append(
            {
                'row_id': examine_date_row['_id'],
                'row': {
                    '故障日期': examine_date_row['_ctime'][:10]
                }
            }
        )
# 将数据集写入实表中。
record_base.batch_update_rows('维修记录', rows_data=increased_common_record_update_data)
# 重新获取新增的维修记录数据。
increased_common_record_list = record_base.list_rows('维修记录')

# 建立更新故障日期的数据集容器。
increased_pic_record_update_data = []

# 如果某行中没有日期key。
for examine_date_row in increased_pic_record_list:
    if not ('故障日期' in examine_date_row):
        # 对该行数据集中添加当前日期。
        increased_common_record_update_data.append(
            {
...
# 将数据集写入实表中。
record_base.batch_update_rows('维修记录(图片版)', rows_data=increased_pic_record_update_data)
# 重新获取新增的维修记录数据。
increased_pic_record_list = record_base.list_rows('维修记录(图片版)')

# 获得需要处理的所有维修记录
increased_record_list = increased_common_record_list + increased_pic_record_list
...
def str_contrast(model1, model2):
    ...
    # 否则，采用如下对比算法来判定是否为同型号。
        ...
        # 使两字符串只保留汉字。
        model1_cn_only = re.sub(r'[\W_0-9a-z]+', '', model1)
        model2_cn_only = re.sub(r'[\W_0-9a-z]+', '', model2)
        """
...
def contrast_name(row1, str1, row2, str2):
    ...
    :return:有效字符长的str，或''
    """
    if (str1 in row1) and (str2 in row2):
...
for record_row_common in increased_record_list[::-1]:
    for common_model_row in existing_common_model_list:
        ...
        if ('普通配件型号' in record_row_common) \
                and \
                ('配件型号' in common_model_row) \
                and \
            ...
            if contrast_result:
                ...
                common_update_data.append(
                    {
                        ...
                        'row': {
                            ...
                            '最近维修/更换时间': record_row_common['故障日期'],
                ...
                关于普通配件型号/名称合并记录的运算
                """
                # 对两行中的配件名称进行避免无key检测，无key会return ''。
                        ...
                        [
                            {
                                ...
                                '配件名称': common_model_name,
                                '最近维修/更换时间': common_model_row['最近维修/更换时间']
                            },
                            {
                ...
                if (not ('重要/核心配件型号' in record_row_common)) \
                        or \
                        record_row_common['重要/核心配件型号'] == '':
                    increased_record_list.remove(record_row_common)
                # 否则，删除此record_row中的'普通配件型号'的key。
                else:
                    del record_row_common['普通配件型号']
...
for record_row_remain in increased_record_list:
    if '普通配件型号' in record_row_remain \
            and \
